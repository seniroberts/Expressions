...
# Convert damages data from string to float and return converted damages as a list.


def damage_values_converter(damages):
    converted_values = []
    multiplier = {"M": 10**6, "B": 10**9}
    for value in range(len(damages)):
        inputvalue = damages[value]
        if inputvalue[-1] in multiplier:
            num = float(inputvalue[:-1])
            factor = multiplier[inputvalue[-1]]  # M / B
            converted_values.append(num * factor)
        else:
            converted_values.append(inputvalue)
    return converted_values


updated_damages = damage_values_converter(damages)

# print(damage_values(damages))
...
def create_hurricane_dict(names, months, years, max_sustained_winds, areas_affected, updated_damages, deaths):
    ...
    for i in range(len(names)):
        hurricanes[names[i]] = {"Name": names[i], "Month": months[i], "Year": years[i],
                                "Max-Sustauned winds": max_sustained_winds[i], "Areas Affected": areas_affected[i], "Damage": updated_damages[i], "Deaths": deaths[i]}
    return hurricanes
...
def create_hurricane_by_year_dict(year):
    hurricane_by_year = []
...
print(create_hurricane_by_year_dict(2018))
